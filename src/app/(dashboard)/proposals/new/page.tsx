...
export default function ProposalNewPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const bizSlug = (tenant?.business_name ?? 'Business')
        .replace(/\s+/g, '')
        .replace(/[^a-zA-Z0-9]/g, '');
      const dateStr = format(new Date(), 'MMMd');
      const fileName = `${bizSlug}-${TEMPLATE_LABELS[template].replace(/\s+/g, '')}-${clientSlug}-${dateStr}.pdf`;
      const businessInfo = tenant
        ...
        : undefined;
      await downloadProposalPDF(template, data, fileName, businessInfo);
