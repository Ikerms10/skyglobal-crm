...
function ChartTooltip({ active, payload, label }: {
  ...
  return (
    <div style={{
      ...
      {payload.map(p => (
        <p key={p.name} style={{ color: p.color, fontWeight: 600, margin: '2px 0', fontFamily: "'DM Mono', monospace" }}>
...
export default function DashboardPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        projectsRes, leadsRes, expensesRes, projectExpensesRes, activitiesRes,
        allProjRes, allExpRes, allProjExpRes,
  ...
  const kpis = [
    { label: 'Total Revenue', value: data?.revenue ?? 0, format: 'currency' as const, color: 'gold' as const, icon: DollarSign, sparkline: data?.revenueSparkline, href: '/reports' },
    { label: 'Gross Profit',  value: data?.profit ?? 0,  format: 'currency' as const, color: 'green' as const, icon: TrendingUp, trend: data?.margin != null ? { value: data.margin, label: 'margin' } : undefined, sparkline: data?.profitSparkline, href: '/reports' },
    { label: 'Active Projects', value: data?.activeProjects ?? 0, format: 'number' as const, color: 'sky' as const, icon: Briefcase, href: '/projects' },
    { label: 'Leads', value: data?.totalLeads ?? 0, format: 'number' as const, color: 'sky' as const, icon: Target, trend: data?.wonLeads != null ? { value: data.wonLeads, label: 'won' } : undefined, href: '/leads' },
  ]
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--sg-text-2)', marginTop: 4, fontFamily: "'DM Mono', monospace" }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '60% 1fr', gap: 16 }} className="grid-cols-1 md:grid-cols-[60%_1fr]">
          ...
          <div style={card}>
            <div style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--sg-text-1)', margin: 0, fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: '-0.01em' }}>
                  Revenue vs Expenses
                </h3>
                <p style={{ fontSize: 11, color: 'var(--sg-text-3)', marginTop: 2, fontFamily: "'DM Mono', monospace" }}>Last 6 months</p>
              </div>
              <div style={{ display: 'flex', gap: 12 }}>
                {[{ c: '#8B6914', n: 'Revenue' }, { c: '#B94A3A', n: 'Expenses' }].map(({ c, n }) => (
                  <div key={n} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--sg-text-3)', fontFamily: "'DM Mono', monospace" }}>{n}</span>
                  </div>
                ))}
              </div>
            </div>
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={data?.chartData ?? []} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                  <XAxis dataKey="month" tick={{ fill: 'var(--sg-text-3)', fontSize: 11, fontFamily: "'DM Mono', monospace" }} axisLine={false} tickLine={false} />
                  <Tooltip content={<ChartTooltip />} />
                  <Line dataKey="revenue" name="Revenue" stroke="#8B6914" strokeWidth={2} dot={false} type="monotone" />
                  <Line dataKey="expenses" name="Expenses" stroke="#B94A3A" strokeWidth={2} dot={false} type="monotone" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                ...
                <p style={{ fontSize: 11, color: 'var(--sg-text-3)', marginTop: 2, fontFamily: "'DM Mono', monospace" }}>
                  {data?.totalLeads ?? 0} leads
                </p>
              </div>
              <Link href="/leads" style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 11, color: 'var(--sg-sky)', textDecoration: 'none', fontFamily: "'DM Mono', monospace" }}>
                View <ArrowRight size={11} />
              </Link>
            </div>
            {isLoading ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {Array.from({ length: 5 }).map((_, i) => <div key={i} className="skeleton" style={{ height: 28 }} />)}
              </div>
            ) : (
                ...
                {Object.entries(data?.stageCounts ?? {}).filter(([, count]) => Number(count) > 0).map(([stage, count]) => (
                  <div key={stage}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 12, color: 'var(--sg-text-2)', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{stage}</span>
                      <span style={{ fontSize: 12, fontWeight: 700, color: STAGE_COLORS[stage] ?? 'var(--sg-text-3)', fontFamily: "'DM Mono', monospace" }}>{count}</span>
                    </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }} className="grid-cols-1 md:grid-cols-3">
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--sg-text-1)', margin: 0, fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: '-0.01em' }}>Recent Activity</h3>
              <Activity size={14} style={{ color: 'var(--sg-sky)' }} />
            </div>
            {isLoading ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {Array.from({ length: 4 }).map((_, i) => <div key={i} className="skeleton" style={{ height: 36 }} />)}
              </div>
                ...
                {(data!.activities as Array<{ id: string; type: string; content: string | null; created_at: string; customer_id: string }>).map(a => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--sg-text-1)', margin: 0, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{a.type}</p>
                      {a.content && <p style={{ fontSize: 11, color: 'var(--sg-text-3)', margin: '1px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.content}</p>}
                    </div>
                    <span style={{ fontSize: 10, color: 'var(--sg-text-3)', flexShrink: 0, fontFamily: "'DM Mono', monospace" }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--sg-text-3)', textAlign: 'center', padding: '20px 0', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>No recent activity</p>
            )}
          </div>
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--sg-text-1)', margin: 0, fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: '-0.01em' }}>Follow-ups</h3>
            ...
            {(data?.followUps ?? []).length > 0 ? (
                ...
                {(data!.followUps as unknown as Array<{ id: string; title: string; follow_up_date: string | null; customers: { name: string } | null }>).map(f => {
                  ...
                  return (
                    <Link key={f.id} href="/leads"
                      ...
                      onMouseEnter={e => { e.currentTarget.style.background = 'var(--c-card-hover)'; e.currentTarget.style.borderColor = 'var(--c-border-mid)' }}
                      onMouseLeave={e => { e.currentTarget.style.background = 'var(--c-sage-bg)'; e.currentTarget.style.borderColor = 'var(--c-sage-border)' }}
                    >
                      <div style={{ minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--sg-text-1)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{f.title}</p>
                        <p style={{ fontSize: 11, color: 'var(--sg-text-3)', margin: 0 }}>{(f.customers as { name: string } | null)?.name}</p>
                      </div>
                      <span style={{
                        fontSize: 11,
              ...
              <p style={{ fontSize: 13, color: 'var(--sg-text-3)', textAlign: 'center', padding: '20px 0', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>No upcoming follow-ups</p>
            )}
          </div>
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--sg-text-1)', margin: 0, fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: '-0.01em' }}>Overdue Payments</h3>
            ...
            {(data?.overdue ?? []).length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {(data!.overdue as unknown as Array<{ id: string; title: string; customer_id: string; contract_value: number | null; amount_paid: number; customers: { name: string } | null }>).map(p => (
                  <Link key={p.id} href={`/customers/${p.customer_id}/projects/${p.id}`}
                    style={{
                      display: 'flex',
                      justifyContent: 'space-between',
                      alignItems: 'center',
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--sg-text-1)', margin: 0, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{p.title}</p>
                      <p style={{ fontSize: 11, color: 'var(--sg-text-3)', margin: 0 }}>{(p.customers as { name: string } | null)?.name}</p>
                    </div>
                    <span style={{ fontSize: 12, color: 'var(--sg-danger)', fontWeight: 700, flexShrink: 0, marginLeft: 8, fontFamily: "'DM Mono', monospace" }}>
              ...
              <>
                <p style={{ fontSize: 13, color: 'var(--sg-success)', textAlign: 'center', padding: '12px 0', fontFamily: "'DM Mono', monospace" }}>
                  &gt; No overdue payments
                </p>
                {(data?.activeProjectsList ?? []).length > 0 && (
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', margin: '12px 0 10px', paddingTop: 12, borderTop: '1px solid var(--sg-border)' }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--sg-text-2)', fontFamily: "'DM Mono', monospace", letterSpacing: '0.08em', textTransform: 'uppercase' }}>Active Projects</span>
                      <Link href="/projects" style={{ fontSize: 11, color: 'var(--sg-sky)', textDecoration: 'none' }}>
                        <ArrowRight size={11} />
                      </Link>
                    </div>
                    {(data!.activeProjectsList as unknown as Array<{ id: string; title: string; contract_value: number | null; customer_id: string; customers: { name: string } | null }>).map(p => (
                      <Link key={p.id} href={`/customers/${p.customer_id}/projects/${p.id}`}
                        style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--sg-border)', textDecoration: 'none' }}
                      >
                        <p style={{ fontSize: 13, color: 'var(--sg-text-1)', margin: 0, fontWeight: 500, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{p.title}</p>
                        <span style={{ fontSize: 12, color: 'var(--sg-gold)', fontWeight: 700, fontFamily: "'DM Mono', monospace" }}>{formatCurrency(p.contract_value)}</span>
                      </Link>
                    ))}
                  </>
                )}
